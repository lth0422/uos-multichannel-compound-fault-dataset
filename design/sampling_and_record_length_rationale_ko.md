...
조건별 synchronized four-channel master record의 수집 길이는 60초로 결정했다. 최종 샘플링레이트, RPM grid, 모델 입력 window 길이, overlap과 안정화 제외 규칙은 아직 확정하지 않았다. 이 문서는 보유 장비, 선행 데이터셋 조사 결과와 물리 기반 진단 요구사항을 함께 검토하는 작업 문서다.

따라서 아래 내용은 최종 contribution이나 최종 수집 조건이 아니라, 교수 검토와 pilot validation 전에 사용할 근거 정리다.

## 선행 데이터셋 조사에서 확인된 점

현재까지 검토한 회전기계·베어링 데이터셋의 진동 샘플링레이트는 약 6.4 kHz부터 200 kHz까지 다양하다. 예를 들어 Arkansas 2023은 6.4 kHz, NASA IMS는 20 kHz, XJTU-SY, KAIST Batch, HUST China/HUSTbearing은 25.6 kHz, Paderborn은 64 kHz를 사용한다. [ARK23-E07, IMS-E05, XJTU-E04, PU-E07, KAIB-E05, KAIB-E06, HUSTC-E12]
...
HUST China/HUSTbearing은 RPM 조건 설계 관점에서 별도로 참고할 가치가 있다. 공식 저장소 README 기준으로 9개 health state를 11개 speed condition에서 수집했으며, 그중 10개는 고정 속도 조건이고 1개는 `0-40-0 Hz` time-varying condition이다. Hz를 shaft rotational frequency로 해석하면 고정 조건은 1,200-4,800 RPM 범위에 해당한다. 따라서 HUSTbearing은 “여러 RPM 조건별로 같은 fault taxonomy를 반복 측정한다”는 점에서 UOS v2의 fixed-speed grid 설계와 유사한 비교 사례로 볼 수 있다. 다만 HUSTbearing은 ER-16K 단일 베어링 중심의 데이터셋이고, 공식 README에서 확인되는 내부 복합 결함은 IR+OR 조합이며, UOS v2가 검토 중인 bearing-rotor compound 조건과 4채널 multi-position synchronized layout의 직접 선행 사례로 보기는 어렵다. [HUSTC-E10-HUSTC-E14]
...
| HUST China / HUSTbearing | 25,600 Hz | 1,200; 1,500; 1,800; 2,100; 2,400; 3,600; 3,900; 4,200; 4,500; 4,800 RPM; 0-2,400-0 RPM equivalent time-varying | 10.2 s |
...
UOS v2에서는 60초 master record를 수집하고, 동일 record에서 1초, 2초, 5초, 10초, 30초, 60초 window를 비교한다. 60초는 원신호 보존 길이에 대한 결정이며, 모델 입력에도 60초가 최적이라는 뜻은 아니다.
...
- 각 조건에서 60초 master record를 수집한다.
- 동일 master record에서 deterministic 1, 2, 5, 10, 30, 60초 window를 비교한다.
...
60초 master 수집은 결정된 전략이다. 다만 공개 파일을 그대로 60초 단위로 제공할지, 어떤 canonical window를 함께 제공할지, 첫 안정화 구간을 제외할지는 아직 최종안이 아니다.
...
데이터 길이는 조건당 60초 synchronized master record로 수집한다. 분석에서는 1, 2, 5, 10, 30, 60초 window를 비교하고, canonical model-input 길이는 실제 RPM, bearing characteristic frequency, stationarity, grouped-split 성능과 on-device inference 요구사항을 검토한 뒤 확정한다.
