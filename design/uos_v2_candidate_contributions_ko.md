...
이 표는 현재 조사한 14개 데이터셋과 UOS v2의 계획을 같은 기준으로 비교한 것이다.
...
| 데이터셋 | 내부 2중 3조합¹ | 내부 3중² | 베어링–로터 3종³ | 다위치 진동 | 동시 샘플링 확인 | 동일 신호 채널 비교⁴ | 물리 검증⁵ | 여러 베어링 형식 | 여러 RPM |
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **UOS v2 계획** | **P** | **P** | **P** | **P** | **P** | **P** | **P** | **P** | **P** |
| UOS v1[2] | X | X | O | X | ? | X | O | O | O |
| CWRU[8] | X | X | X | O | ? | X | △ | X | O |
| Paderborn[6] | △ | X | X | X | ? | X | O | X | O |
| XJTU-SY[9] | ? | ? | X | X | ? | X | △ | X | O |
| NASA IMS[7] | △ | X | X | O | ? | X | O | X | X |
| MAFAULDA[5] | X | X | △ | O | ? | △ | △ | X | O |
| Arkansas 2023[3] | △ | X | △ | O | ? | X | X | X | O |
| Ottawa 2018[10] | X | X | X | X | ? | X | △ | X | O |
| Ottawa 2023[11] | X | X | X | X | ? | X | O | X | X |
| PRONOSTIA[12] | △ | △ | X | X | ? | X | O | X | O |
| KAIST Batch[4] | X | X | X | O | O | X | O | X | O |
| HUST Vietnam[1] | O | X | X | X | ? | X | △ | X | △ |
| SEU[13] | △ | X | X | ? | ? | X | X | X | X |
| HUST China[14] | △ | X | X | ? | ? | X | X | X | O |
...
기존 데이터셋은 각 요소를 부분적으로 제공하지만, 현재 검토 범위에서는 표의 모든 요소를 함께 제공한 데이터셋을 확인하지 못했다. 따라서 UOS v2가 계획한 항목을 실제로 모두 수집하고 검증한다면 다음과 같이 설명할 수 있다.

> **기존 데이터셋에 흩어져 있던 복합결함 구성, 다위치 동시 진동, 단일·다채널 비교, 물리 검증을 하나의 일관된 데이터셋으로 제공한다.**
