...
| 수집 묶음 전체 | 구조·시간축, 저속·고속 RMS 경향과 1X가 확인됨. ±50 g 초과 3표본과 낮은 BSF 검출률은 보완사항으로 관리함 | **통과(보완사항 있음)** |
...
- BSF 검출률은 물리 검증 보조 결과이며, 낮은 검출률만으로 수집 묶음을 실패 처리하지 않음
...
- 빨간 실선은 회전주파수 1X, 빨간 점선은 BSF의 1·2·3차 고조파임
- 정규화 그림은 주파수 위치 확인용이며 파일 사이의 절대 진폭 비교에는 사용하지 않음
- 회전주파수의 최종 판정은 포락선이 아니라 원신호 FFT 결과를 사용함

#### 대표 조건: 로터 정상 H

![N204 B, H, 600 RPM, CH0](figures/envelope_fft_ch0/individual/0600rpm_H_B_R1_CH0.png)

![N204 B, H, 1200 RPM, CH0](figures/envelope_fft_ch0/individual/1200rpm_H_B_R1_CH0.png)

![N204 B, H, 1600 RPM, CH0](figures/envelope_fft_ch0/individual/1600rpm_H_B_R1_CH0.png)

- [48개 그림 전체 보기](figures/envelope_fft_ch0/contact_sheet.png)
- 개별 PNG 위치: `figures/envelope_fft_ch0/individual/`
- 그림에 사용한 기준 주파수: [`ch0_envelope_reference_frequencies.csv`](results/ch0_envelope_reference_frequencies.csv)
...
| RMS–RPM 경향 | `Pass` | 32/32조합에서 1,600 RPM RMS가 600 RPM보다 높음. 중간 RPM의 국소 감소는 보조 관찰로 관리함 |
| 회전주파수 1X | `Pass` | 192/192채널에서 확인됨 |
| 롤러 결함주파수 BSF | `보조 검증` | 일부 채널에서 확인됐으며 현재 보수적 기준의 검출률과 특이도는 추가 보정이 필요함 |
| 수집 묶음 전체 | `통과(보완사항 있음)` | 수집 완전성·RMS의 저속/고속 경향·1X가 확인됨. 희소 rail과 BSF 기준 보정은 별도 관리함 |
