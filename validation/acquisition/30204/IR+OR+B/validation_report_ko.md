...
| 수집 묶음 전체 | 구조·시간축, 저속·고속 RMS 경향과 파일 단위 1X가 확인됨. ±50 g 초과와 B 검출 문제는 보완사항으로 관리함 | **통과(보완사항 있음)** |
...
- 현재 단계의 주 판정은 수집 완전성, RMS의 전체적인 RPM 경향과 회전주파수 1X로 정함
- IR·OR·B 검출률은 물리 검증 보조 결과이며, 세 성분이 모두 검출되지 않았다는 이유만으로 수집 묶음을 실패 처리하지 않음
...
- 48개 파일의 CH0에 동일한 2~7 kHz 4차 Butterworth 대역통과 필터를 적용함
- 각 파일의 원신호 60~180초를 사용하고 Hilbert 포락선 FFT를 계산함
- 파란색 곡선은 파일별 최댓값을 1로 맞춘 포락선 스펙트럼임
- 빨간 실선은 회전주파수 1X임
- 빨간 파선·일점쇄선·점선은 각각 BPFI·BPFO·BSF의 1·2·3차 고조파임
- 자홍색 선은 BPFI·BPFO·BSF 쌍별 합·차 주파수 후보임
- 합·차 주파수는 비선형 상호작용의 보조 후보이며 복합 결함을 단독으로 증명하지 않음
- 정규화 그림은 주파수 위치 확인용이며 파일 사이의 절대 진폭 비교에는 사용하지 않음
- 회전주파수의 최종 판정은 포락선이 아니라 원신호 FFT 결과를 사용함

#### 대표 조건: 로터 정상 H

![30204 IR+OR+B, H, 600 RPM, CH0](figures/envelope_fft_ch0/individual/0600rpm_H_IR+OR+B_R1_CH0.png)

![30204 IR+OR+B, H, 1200 RPM, CH0](figures/envelope_fft_ch0/individual/1200rpm_H_IR+OR+B_R1_CH0.png)

![30204 IR+OR+B, H, 1600 RPM, CH0](figures/envelope_fft_ch0/individual/1600rpm_H_IR+OR+B_R1_CH0.png)

- [48개 그림 전체 보기](figures/envelope_fft_ch0/contact_sheet.png)
- 개별 PNG 위치: `figures/envelope_fft_ch0/individual/`
- 그림에 사용한 기본·합차 기준 주파수: [`ch0_envelope_reference_frequencies.csv`](results/ch0_envelope_reference_frequencies.csv)
...
| RMS–RPM 경향 | `Pass` | 32/32조합에서 1,600 RPM RMS가 600 RPM보다 높음 |
| 회전주파수 1X | `Pass` | 48/48파일에서 최소 3개 채널이 기준을 통과함 |
| IR 구성 성분 | `보조 검증` | 70/192채널에서 보수적 기준을 통과함 |
| OR 구성 성분 | `보조 검증` | 107/192채널에서 보수적 기준을 통과함 |
| B 구성 성분 | `보조 검증 미완료` | 두 고조파 기준 통과가 0/192채널이며 단일 결함 대조가 필요함 |
| 수집 묶음 전체 | `통과(보완사항 있음)` | 수집 완전성·RMS의 저속/고속 경향·파일 단위 1X가 확인됨. clipping 정책과 B 대조 검증은 별도 관리함 |
