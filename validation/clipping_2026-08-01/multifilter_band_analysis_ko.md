...
- 대상 규모: 26개 파일·47개 채널임
- 채널 구성: CH0 25개·CH1 22개임
- 원자료 보존: TDMS 파일을 수정하지 않고 파생 CSV와 그림만 생성함

## 1. 먼저 내릴 수 있는 판단

- 분석 대상 47채널 중 46채널이 ADC의 같은 디지털 레일에 실제 도달함
- 이미 잘린 파형이므로 필터 이전의 실제 순간 진폭과 정확한 주파수별 진폭 복원 불가능함
- 0~2, 7~10, 10~11.5, 11.5~12.8 kHz 구간에서는 어떤 필터도 50 g 초과 결과를 만들지 않음
- 2~7 kHz 구간에서 N204·1600 RPM·로터 정상(H)·베어링 `IR+OR+B`·CH0 한 채널만 50 g 초과 결과를 보임
- 위 한 채널은 Butterworth 52.54 g, Chebyshev I 50.46 g, Elliptic 50.78 g임
- 같은 채널에서 Chebyshev II는 31.51 g, Bessel은 39.62 g임
...
- 각 채널의 전체 기록에서 절대값이 가장 큰 sample 위치를 찾음
- 해당 최대값을 포함하도록 10초 구간을 추출함
- 최대값이 기록 시작·끝에 가까우면 가능한 범위 안에서 10초 구간을 이동함
- 각 필터·각 구간에 대해 다음 값을 계산함
  - 절대 최대값임
  - 절대값 상위 0.1% 경계인 p99.9임
  - 절대값 상위 0.01% 경계인 p99.99임
  - RMS임
  - 원신호 10초 RMS 제곱 대비 대역 RMS 제곱 비율임
...
- 분석 대상 분모: 모든 셀에서 47채널임
- 오른쪽 셀의 숫자: 47개 채널별 최대 절대값의 중앙값임
- 중앙값은 47개 중 가운데에 해당하는 전형적인 크기이며 단일 극단값의 영향을 줄인 값임

| 필터 | 0~2 kHz | 2~7 kHz | 7~10 kHz | 10~11.5 kHz | 11.5~12.8 kHz |
|---|---:|---:|---:|---:|---:|
| Butterworth | 0 | 1 | 0 | 0 | 0 |
| Chebyshev I | 0 | 1 | 0 | 0 | 0 |
...
- 회전수: 1600 RPM임
- 로터 조건: H, 로터 정상 조건임
- 베어링 결함: `IR+OR+B`, 내륜+외륜+롤러 3중 결함임
- 채널: CH0임
- 주파수 구간: 2~7 kHz임
- 원신호 절대 최대값: 53.14 g임
- 원신호의 ADC rail sample: 존재함

| 필터 | 절대 최대값 | p99.9 | p99.99 | RMS | 원신호 대비 대역 에너지 비율 |
|---|---:|---:|---:|---:|---:|
| Butterworth | 52.54 g | 27.45 g | 38.04 g | 3.32 g | 62.07% |
| Chebyshev I | 50.46 g | 26.69 g | 37.25 g | 3.20 g | 57.80% |
| Chebyshev II | 31.51 g | 18.46 g | 25.25 g | 2.17 g | 26.63% |
| Elliptic | 50.78 g | 26.81 g | 37.64 g | 3.22 g | 58.33% |
| Bessel | 39.62 g | 21.14 g | 29.24 g | 2.48 g | 34.53% |

### 해석

- 2~7 kHz 성분이 해당 채널의 큰 순간 피크에 중요한 영향을 준 정황임
- p99.9가 18.46~27.45 g이고 RMS가 2.17~3.32 g이므로 50 g 초과는 지속 진동이 아니라 극히 짧은 사건임
- 필터 출력 최대값이 필터에 따라 31.51~52.54 g로 변함
- 이미 잘린 파형 주변에서 필터가 만든 재구성 진동과 경계 응답이 포함됨
- 필터 후 최대값을 센서 입력단에서 실제 52.54 g가 발생했다는 증거로 사용하면 안 됨
...
- 2~7 kHz 대역통과에서 1채널만 50 g를 넘었다고 해서 0~10 kHz 누적 신호도 1채널만 넘는 것은 아님
...
|---|---:|---:|---:|---:|---:|---:|
| 원신호 | 47 | 47 | 47 | 47 | 47 | 47 |
| 약 0~11.5 kHz | 23 | 18 | 12 | 18 | 8 | 18 |
| 약 0~10 kHz | 10 | 9 | 4 | 10 | 1 | 9 |
| 약 0~7 kHz | 1 | 1 | 0 | 1 | 0 | 1 |
| 약 0~2 kHz | 0 | 0 | 0 | 0 | 0 | 0 |

### 10 kHz 이상을 줄였을 때의 직접 답변

- 저 한 채널을 제외한 모든 채널이 50 g 안으로 들어오는 결과가 아님
- Butterworth 기준 10채널이 여전히 50 g를 넘음
- Chebyshev I 기준 9채널이 여전히 50 g를 넘음
- Chebyshev II 기준 4채널이 여전히 50 g를 넘음
- Elliptic 기준 10채널이 여전히 50 g를 넘음
- Bessel 기준 1채널이 여전히 50 g를 넘음
- 필터 5종 중 3종 이상이 동의한 채널은 9개임
- 필터 종류에 따라 1~10채널로 차이가 크므로 단일 필터 결과만으로 장비 범위 결정 불가능함

### 10 kHz 저역통과 후 필터 3종 이상이 50 g 초과에 동의한 9조건
...
- 46/47채널이 이미 ADC rail에 도달한 뒤의 파형임
...
|---|---|---:|---|
| 내륜 IR | 2~7 kHz | 53.2~55.3% | BPFI 계열 carrier 후보로 가장 안정적임 |
| 외륜 OR | 2~7 kHz | 31.9~34.0% | 다른 구간보다 높지만 전체 검출률은 낮음 |
| 볼·롤러 B | 7~10 kHz | 46.8~53.2% | BSF·FTF 계열 carrier 후보로 가장 안정적임 |
| 세 계열 동시 | 2~7 kHz | 필터별 6~7채널 | 3중 결함이 모든 채널에서 세 계열을 동시에 보장하지 않음 |

### 필터 다수결 결과

- 한 채널·대역·결함 계열에서 필터 5종 중 3종 이상이 15 dB 검출한 경우를 다수결 검출로 계산함
- 0~2 kHz에서 IR·OR·B 세 계열이 모두 검출된 채널: 5/47개임
- 2~7 kHz에서 IR·OR·B 세 계열이 모두 검출된 채널: 7/47개임
- 7~10 kHz에서 IR·OR·B 세 계열이 모두 검출된 채널: 5/47개임
- 10~11.5 kHz에서 IR·OR·B 세 계열이 모두 검출된 채널: 3/47개임
- 2~7 kHz에서 세 계열 중 하나 이상 검출된 채널: 40/47개임
- 위 결과는 47개의 독립 실험을 의미하지 않음
...
| 파일 | 내용 |
|---|---|
| `results/multifilter_band_metrics.csv` | 47채널×5필터×5대역의 최대값·분위수·RMS·에너지 결과임 |
...
| `figures/multifilter_band_peak_summary.png` | 필터별 50 g 초과 수와 최대값 중앙값 그림임 |
...
- 10 kHz 저역통과 후에도 필터 다수결 기준 9채널이 50 g를 넘음
